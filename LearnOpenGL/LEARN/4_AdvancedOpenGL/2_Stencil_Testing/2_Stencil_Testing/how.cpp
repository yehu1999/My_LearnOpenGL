...
namespace how
{
	...
	//每个窗口库都需要为你配置一个模板缓冲。GLFW自动做了这件事，所以我们不需要告诉GLFW来创建一个，
	...
	void how1()
	{
		...
		glStencilMask(0xFF); // 每一位写入模板缓冲时都保持原样（开启写入）
	...
	void how2()
	{
		...
		*/
		...
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);//清空上一帧缓冲

		glStencilMask(0x00); //绘制地板时禁用模板写入
		//normalShader.use();
		//DrawFloor()

		glStencilFunc(GL_ALWAYS, 1, 0xFF);//设置模板函数(测试条件:全部，参考值:1)
		glStencilMask(0xFF);//启用模板写入
		//DrawTwoContainers();

		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);//设置模板函数(测试条件:不等于参考值，参考值:1)
		...
		//你可以看到这两个箱子的边框重合了，这通常都是我们想要的结果（想想策略游戏中，我们希望选择10个单位，合并边框通常是我们想需要的结果）。
