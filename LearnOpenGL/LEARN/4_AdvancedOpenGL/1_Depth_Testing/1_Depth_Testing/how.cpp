...
namespace how
{
	...
	//在坐标系统小节中，我们渲染了一个3D箱子，并且运用了深度缓冲(Depth Buffer)来防止被阻挡的面渲染到其它面的前面。
	//在这一节中，我们将会更加深入地讨论这些储存在深度缓冲（或z缓冲(z-buffer)）中的深度值(Depth Value)，
	//以及它们是如何确定一个片段是处于其它片段后方的。

	//深度缓冲就像颜色缓冲(Col or Buffer)（储存所有的片段颜色：视觉输出）一样，在每个片段中储存了信息，并且（通常）和颜色缓冲有着一样的宽度和高度。
	...
	void how()
	{
		...
		*/
		...
		//让我们看看改变深度函数会对视觉输出有什么影响。
		//我们将使用一个新的代码配置，它会显示一个没有光照的基本场景，里面有两个有纹理的立方体，放置在一个有纹理的地板上。
		//测试代码在test.cpp中

		//在源代码中，我们将深度函数改为GL_ALWAYS：
		glEnable(GL_DEPTH_TEST);
		glDepthFunc(GL_ALWAYS);
		//这将会模拟我们没有启用深度测试时所得到的结果。深度测试将会永远通过，所以最后绘制的片段将会总是会渲染在之前绘制片段的上面，
		...
		//这里的near和far值是我们之前提供给投影矩阵设置可视平截头体的（见坐标系统）那个near和far值。
		//这个方程需要平截头体中的一个z值，并将它变换到了[0, 1]的范围中。
		...
		//由于非线性方程与 1/z 成正比，在1.0和2.0之间的z值将会变换至1.0到0.5之间的深度值，这就是一个float提供给我们的一半精度了，
		//这在z值很小的情况下提供了非常大的精度。在50.0和100.0之间的z值将会只占2%的float精度，这正是我们所需要的。
		//这样的一个考虑了远近距离的方程是这样的：
		//Fdepth = (1/z − 1/near) / (1/far − 1/near)
		//如果你不知道这个方程是怎么回事也不用担心。
		...
		//如果你想深度了解投影矩阵究竟做了什么，我建议阅读这篇文章 http://www.songho.ca/opengl/gl_projectionmatrix.html
	...
	*/
	//如果你再次运行程序的话，你可能会注意到所有东西都是白色的，
	//看起来就想我们所有的深度值都是最大的1.0。所以为什么没有靠近0.0（即变暗）的深度值呢？

	//你可能还记得在上一部分中说到，屏幕空间中的深度值是非线性的，
	//即它在z值很小的时候有很高的精度，而z值很大的时候有较低的精度。
	//片段的深度值会随着距离迅速增加，所以几乎所有的顶点的深度值都是接近于1.0的。
	//如果我们小心地靠近物体，你可能会最终注意到颜色会渐渐变暗，显示它们的z值在逐渐变小

	//这很清楚地展示了深度值的非线性性质。
	//近处的物体比起远处的物体对深度值有着更大的影响。
	//只需要移动几厘米就能让颜色从暗完全变白。

	//然而，我们也可以让片段非线性的深度值变换为线性的。
	//要实现这个，我们需要仅仅反转深度值的投影变换。
	//这也就意味着我们需要首先将深度值从[0, 1]范围重新变换到[-1, 1]范围的标准化设备坐标（裁剪空间）。
	//接下来我们需要像投影矩阵那样反转这个非线性方程（方程2），并将这个反转的方程应用到最终的深度值上。
	//最终的结果就是一个线性的深度值了。听起来是可行的，对吧？
	...
	//这篇注重数学的文章为感兴趣的读者详细解释了投影矩阵，它也展示了这些方程是怎么来的 http://www.songho.ca/opengl/gl_projectionmatrix.html
	...
	*/
	...
	//如果我们现在运行程序，我们就能看见深度值随着距离增大是线性的了。
	//尝试在场景中移动，看看深度值是怎样以线性变化的。
	//颜色大部分都是黑色，因为深度值的范围是0.1的近平面到100的远平面，它离我们还是非常远的。
	//结果就是，我们相对靠近近平面，所以会得到更低的（更暗的）深度值。

	//深度冲突
	/*
	一个很常见的视觉错误会在两个平面或者三角形非常紧密地平行排列在一起时会发生，
	深度缓冲没有足够的精度来决定两个形状哪个在前面。结果就是这两个形状不断地在切换前后顺序，这会导致很奇怪的花纹。
	...
	第二个技巧是尽可能将近平面设置远一些。(test.cpp中已将cube的y值设为0.001f，解决了深度冲突问题)
	...
	我们上面讨论的三个技术是最普遍也是很容易实现的抗深度冲突技术了。
	还有一些更复杂的技术，但它们依然不能完全消除深度冲突。
