...
Shader skyboxShader("", "");
unsigned int skyboxVAO, cubeVAO, skyboxTexture;
Camera camera;
...
namespace how
{
	...
	//一个有纹理的立方体。你可能会奇怪，这样一个立方体有什么用途呢？
	//为什么要把6张纹理合并到一张纹理中，而不是直接使用6个单独的纹理呢？
	...
	void how()
	{
		...
		*/
		...
		//和OpenGL的很多枚举(Enum)一样，它们背后的int值是线性递增的，
		//所以如果我们有一个纹理位置的数组或者vector，我们就可以从GL_TEXTURE_CUBE_MAP_POSITIVE_X开始遍历它们，
		...
		//不要被GL_TEXTURE_WRAP_R吓到，它仅仅是为纹理的R坐标设置了环绕方式，
		//它对应的是纹理的第三个维度（和位置的z一样）。
		...
		//在片段着色器中，我们使用了一个不同类型的采样器，samplerCube，我们将使用texture函数使用它进行采样，
		//但这次我们将使用一个vec3的方向向量而不是vec2。使用立方体贴图的片段着色器会像是这样的：
		/*
	...
	//游戏中使用天空盒的例子有群山、白云或星空。下面这张截图中展示的是星空的天空盒，它来自于『上古卷轴3』：
	// https://learnopengl-cn.github.io/img/04/06/cubemaps_morrowind.jpg
	//你可能现在已经猜到了，立方体贴图能完美满足天空盒的需求：
	//我们有一个6面的立方体，每个面都需要一个纹理。
	//在上面的图片中，他们使用了夜空的几张图片，让玩家产生其位于广袤宇宙中的错觉，
	//但实际上他只是在一个小小的六面体盒子当中。

	//如果你将这六个面折成一个立方体，你就会得到一个完全贴图的立方体，模拟一个巨大的场景。
	//一些资源可能会提供了这样格式的天空盒，你必须手动提取六个面的图像，但在大部分情况下它们都是6张单独的纹理图像。

	//之后我们将在场景中使用（高质量的）天空盒

	//加载天空盒

	//因为天空盒本身就是一个立方体贴图，加载天空盒和之前加载立方体贴图时并没有什么不同。
	...
	void how()
	{
		...
		//要渲染天空盒的话，我们需要一组新的着色器，它们都不是很复杂。
		//因为我们只有一个顶点属性，顶点着色器非常简单：
		/*
		#version 330 core
		...
		*/
		...
		//绘制天空盒时，我们需要将它变为场景中的第一个渲染的物体，并且禁用深度写入。
		...
		//你可能还记得在基础光照小节中，我们通过取4x4矩阵左上角的3x3矩阵来移除变换矩阵的位移部分。
	...
	//优化
	/*
	目前我们是首先渲染天空盒，之后再渲染场景中的其它物体。
	这样子能够工作，但不是非常高效。如果我们先渲染天空盒，我们就会对屏幕上的每一个像素运行一遍片段着色器，即便只有一小部分的天空盒最终是可见的。
	...
	我们还要改变一下深度函数，将它从默认的GL_LESS改为GL_LEQUAL(小于等于参考值时通过测试)。
	...
	*/
	...
	void how()
	{
		...
		//我们可以使用GLSL内建的reflect函数来计算这个反射向量。
		//最终的R¯向量将会作为索引来采样立方体贴图的方向向量，返回环境的颜色值。
		//最终的结果是物体看起来反射了天空盒。

		//因为我们已经在场景中配置好天空盒了，创建反射效果并不会很难。
		//我们将会改变箱子的片段着色器，让箱子有反射性：
		/*
		#version 330 core
		out vec4 FragColor;
		...
		*/
		...
		//我们现在使用了一个法向量，所以我们将再次使用法线矩阵(Normal Matrix)来变换它们。Position输出向量是一个世界空间的位置向量。
		//顶点着色器的这个Position输出将用来在片段着色器内计算观察方向向量。

		//因为我们使用了法线，你还需要更新一下顶点数据，并更新属性指针。
		//还要记得去设置cameraPos这个uniform。
		...
		//与漫反射和镜面光贴图一样，反射贴图也是可以采样的纹理图像，它决定这片段的反射性。
		//通过使用反射贴图，我们可以知道模型的哪些部分该以什么强度显示反射。
		//在本节的练习中，将由你来为我们之前创建的模型加载器中引入反射贴图，显著提升纳米装模型的细节。
	
		//有几点要注意的：
		//Assimp在大多数格式中都不太喜欢反射贴图，所以我们需要欺骗一下它，将反射贴图储存为漫反射贴图。
		//你可以在加载材质的时候将反射贴图的纹理类型设置为aiTextureType_AMBIENT。
	...
	void how()
	{
		...
		//同样，我们有一个观察向量I¯，一个法向量N¯，而这次是折射向量R¯。
		//可以看到，观察向量的方向轻微弯曲了。
		//弯折后的向量R¯将会用来从立方体贴图中采样。
		...
		*/
		...
		//我们使用这些折射率来计算光传播的两种材质间的比值。
		//在我们的例子中，光线/视线从空气进入玻璃（如果我们假设箱子是玻璃制的），
		//所以比值为1.00 / 1.52 = 0.658
		...
		*/
		...
		//编译程序并运行，但结果并不是很有趣，因为我们只使用了一个简单的箱子，它不太能显示折射的效果，现在看起来只是有点像一个放大镜。
		//对纳米装使用相同的着色器却能够展现出了我们期待的效果：一个类玻璃的物体。
	
		//你可以想象出有了光照、反射、折射和顶点移动的正确组合，你可以创建出非常漂亮的水。
		//注意，如果要想获得物理上精确的结果，我们还需要在光线离开物体的时候再次折射，现在我们使用的只是单面折射(Single-side Refraction)，但它对大部分场合都是没问题的。
	}
	...
	这就叫做动态环境映射(Dynamic Environment Mapping)，因为我们动态创建了物体周围的立方体贴图，并将其用作环境贴图。
