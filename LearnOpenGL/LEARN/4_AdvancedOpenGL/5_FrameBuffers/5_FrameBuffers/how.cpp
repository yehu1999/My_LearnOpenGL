...
namespace how
{
	...
	void how()
	{
		...
		*/
		...
		//既然我们已经知道帧缓冲（大概）是怎么工作的了，是们时候实践它们了。
		//我将会将场景渲染到一个附加到帧缓冲对象上的颜色纹理中，之后将在一个横跨整个屏幕的四边形上绘制这个纹理。
		...
		*/
		...
		//我们将不会包含任何花哨的矩阵变换，因为我们提供的是标准化设备坐标的顶点坐标，所以我们可以直接将它们设定为顶点着色器的输出。
		//顶点着色器是这样的：
		/*
		#version 330 core
		...
		*/
		...
		//第二，当绘制四边形时，我们将禁用深度测试，因为我们是在绘制一个简单的四边形，并不需要关系深度测试。
		//在绘制普通场景的时候我们将会重新启用深度测试。

		//有很多步骤都可能会出错，所以如果你没有得到输出的话，尝试调试程序，并重新阅读本节的相关部分。
		//如果所有的东西都能够正常工作，你将会得到下面这样的视觉输出：
		//一个看似普通，但实际上是把渲染缓冲绘制在一个覆盖屏幕的四边形上的画面
	}
	...
	*/
	//在片段着色器中，我们首先为周围的纹理坐标创建了一个9个vec2偏移量的数组。
	//偏移量是一个常量，你可以按照你的喜好自定义它。
	//之后我们定义一个核，在这个例子中是一个锐化(Sharpen)核，它会采样周围的所有像素，锐化每个颜色值。
	...
	*/
	...
	//模糊也能够让我们来平滑颜色值，我们将在之后教程中使用到。

	//你可以看到，只要我们有了这个核的实现，创建炫酷的后期处理特效是非常容易的事。
	//我们再来看最后一个很流行的效果来结束本节的讨论。
