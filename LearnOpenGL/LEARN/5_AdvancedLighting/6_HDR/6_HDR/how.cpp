...
GLuint colorBuffer, hdrColorBufferTexture,hdrFBO;
...
namespace how
{
	...
	void what()
	{
		...
		//一个更好的方案是让颜色暂时超过1.0，然后将其转换至0.0到1.0的区间内，
		//从而防止损失细节。
		...
		//我们能做的是用一个不同的方程与/或曲线来转换这些HDR值到LDR值，
	...
	void how()
	{
		//在实现HDR渲染之前，我们首先需要一些方法来防止颜色值在每次片段着色器运行后被限制约束。
		//当帧缓冲使用了一个标准化的定点颜色格式(像GL_RGB)为其颜色缓冲的内部格式，
		...
		//在这个教程的例子当中，我们先渲染一个光照的场景到浮点帧缓冲中，
		//之后再在一个铺屏四边形(Screen-filling Quad)上应用这个帧缓冲的颜色缓冲，代码会是这样子：
		...
		*/
		...
		//然而，这个2D四边形的输出是被直接渲染到默认的帧缓冲中，导致所有片段着色器的输出值被约束在0.0到1.0间，
		...
		//为了修复这个问题我们需要做的是无损转化所有浮点颜色值回0.0-1.0范围中。
		//我们需要应用到色调映射。
	}
	...
	void how()
	{
		...
		*/
		...
		//用了这个非常简单地色调映射算法，我们可以合适的看到存在浮点帧缓冲中整个范围的HDR值，
		//使我们能在不丢失细节的前提下，对场景光照有精确的控制。
	}
	...
	void how()
	{
		...
		*/
		...
		//在这里我们将exposure定义为默认为1.0的uniform，
		//从而允许我们更加精确设定我们是要注重黑暗还是明亮的区域的HDR颜色值。
		//举例来说，高曝光值会使隧道的黑暗部分显示更多的细节，
		//然而低曝光值会显著减少黑暗区域的细节，但允许我们看到更多明亮区域的细节。
		//下面这组图片展示了在不同曝光值下的通道：https://learnopengl-cn.github.io/img/05/06/hdr_exposure.png
		//这个图片清晰地展示了HDR渲染的优点。
		//通过改变曝光等级，我们可以看见场景的很多细节，而这些细节可能在LDR渲染中都被丢失了。
		//比如说隧道尽头，在正常曝光下木头结构隐约可见，但用低曝光木头的花纹就可以清晰看见了。
		//对于近处的木头花纹来说，在高曝光下会能更好的看见。
	}
	...
	也有一些技巧被称作自动曝光调整(Automatic Exposure Adjustment)或者叫人眼适应(Eye Adaptation)技术，
	它能够检测前一帧场景的亮度并且缓慢调整曝光参数模仿人眼，使得场景在黑暗区域逐渐变亮或者在明亮区域逐渐变暗。
	
	HDR渲染的真正优点在庞大和复杂的场景中应用复杂光照算法会被显示出来，
	但是出于教学目的创建这样复杂的演示场景是很困难的，这个教程用的场景是很小的，而且缺乏细节。
	但是如此简单的演示也是能够显示出HDR渲染的一些优点：
	在明亮和黑暗区域无细节损失，因为它们可以通过色调映射重新获得；
	多个光照的叠加不会导致亮度被截断的区域的出现，光照可以被设定为它们原来的亮度值而不是被LDR值限制。
	而且，HDR渲染也使一些有趣的效果更加可行和真实; 
	其中一个效果叫做泛光(Bloom)，我们将在下一节讨论。
