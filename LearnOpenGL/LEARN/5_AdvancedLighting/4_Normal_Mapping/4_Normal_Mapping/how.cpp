...
namespace how
{
	...
	//纹理确有助益，然而当你近看它们时，这个事实便隐藏不住了。
	//现实中的物体表面并非是平坦的，而是表现出无数（凹凸不平的）细节。

	//例如，砖块的表面。砖块的表面非常粗糙，显然不是完全平坦的：
	//它包含着接缝处水泥凹痕，以及非常多的细小的空洞。
	...
	//答案会是表面的法线向量。以光照算法的视角考虑的话，只有一件事决定物体的形状，这就是垂直于它的法线向量。
	//砖块表面只有一个法线向量，表面完全根据这个法线向量被以一致的方式照亮。
	//如果每个fragment都是用自己的不同的法线会怎样？这样我们就可以根据表面细微的细节对法线向量进行改变；
	//这样就会获得一种表面看起来要复杂得多的幻觉：https://learnopengl-cn.github.io/img/05/04/normal_mapping_surfaces.png
	...
	//应用到砖墙上，效果像这样：https://learnopengl-cn.github.io/img/05/04/normal_mapping_compare.png
	//你可以看到细节获得了极大提升，开销却不大。
	//因为我们只需要改变每个fragment的法线向量，并不需要改变所有光照公式。
	//现在我们是为每个fragment传递一个法线，不再使用插值表面法线。
	//这样光照使表面拥有了自己的细节。
	...
	void how()
	{
		...
		// vec3 rgb_normal = normal * 0.5 + 0.5; // 将法线向量从 [-1,1] 转换至 [0,1]

		//将法线向量变换为像这样的RGB颜色元素，我们就能把根据表面的形状的fragment的法线保存在2D纹理中。
		//教程开头展示的那个砖块的例子的法线贴图如下所示：https://learnopengl-cn.github.io/img/05/04/normal_mapping_normal_map.png
		...
		//要注意的是这个链接里的法线贴图和上面展示的那个不一样。
		//原因是OpenGL读取的纹理的y（或V）坐标和纹理通常被创建的方式相反。
		//链接里的法线贴图的y（或绿色）元素是相反的（你可以看到绿色现在在下边）；
		//如果你没考虑这个，光照就不正确了
		//（译注：如果你现在不再使用SOIL了，那就不要用链接里的那个法线贴图，这个问题是SOIL载入纹理上下颠倒所致，它也会把法线在y方向上颠倒）。
		...
		*/
		...
		//这里我们将被采样的法线颜色从0到1重新映射回-1到1，便能将RGB颜色重新处理成法线，
		//然后使用采样出的法线向量应用于光照的计算。在例子中我们使用的是Blinn-Phong着色器。

		//通过慢慢随着时间慢慢移动光源，你就能明白法线贴图是什么意思了。
		//运行这个例子你就能得到本教程开始的那个效果。

		//然而有个问题限制了刚才讲的那种法线贴图的使用。
		//我们使用的那个法线贴图里面的所有法线向量都是指向正z方向的(即竖直并对着我们放置)。
		//上面的例子能用，是因为那个平面的表面法线也是指向正z方向的。
		//可是，如果我们在表面法线指向正y方向(即水平，面朝上放置)的平面上使用同一个法线贴图会发生什么？
		// https://learnopengl-cn.github.io/img/05/04/normal_mapping_correct.png
		//光照看起来完全不对！
		//发生这种情况是因为平面的表面法线现在指向了y，而采样得到的法线仍然指向的是z。
		//结果就是光照仍然认为表面法线和之前朝向正z方向时一样；这样光照就不对了。
		//下面的图片展示了这个表面上采样的法线的近似情况：https://learnopengl-cn.github.io/img/05/04/normal_mapping_ground_normals.png

		//你可以看到所有法线都指向z方向，它们本该朝着表面法线指向y方向的。一个可行方案是为每个表面制作一个单独的法线贴图。
		//如果是一个立方体的话我们就需要6个法线贴图，但是如果模型上有无数的朝向不同方向的表面，这就不可行了
		//（译注：实际上对于复杂模型可以把朝向各个方向的法线储存在同一张贴图上，你可能看到过不只是蓝色的法线贴图，不过用那样的法线贴图有个问题是你必须记住模型的起始朝向，如果模型运动了还要记录模型的变换，这是非常不方便的；
		//此外就像作者所说的，如果把一个diffuse纹理应用在同一个物体的不同表面上，就像立方体那样的，就需要做6个法线贴图，这也不可取）。
	...
	void how()
	{
		...
		//切线空间是三角形表面的局部空间：法线相对于各个三角形的局部参考系。
		//它就像法线贴图向量的局部空间；它们都被定义为指向正z方向，无论最终变换到什么方向。
		//使用一个特定的矩阵，我们就能将切线空间中的法线向量转成世界或视图空间下，使它们转向到最终的贴图表面的方向。

		//我们可以说，上个部分那个朝向正y的法线贴图错误的贴到了表面上。
		//法线贴图被定义在切线空间中，所以一种解决问题的方式是计算出一种矩阵，
		//把法线从切线空间变换到一个不同的空间，这样它们就能和表面法线方向对齐了：法线向量都会指向正y方向。
		//切线空间的一大好处是我们可以为任何类型的表面计算出一个这样的矩阵，由此我们可以把切线空间的z方向和表面的法线方向对齐。

		//这种矩阵叫做TBN矩阵这三个字母分别代表切线Tangent、副切线Bitangent/Binormal和法线Normal向量。这是建构这个矩阵所需的向量。
		//要建构这样一个把切线空间转变为不同空间的变异矩阵，我们需要三个相互垂直的向量，
		//它们沿一个表面的法线贴图对齐于：上、右、前；这和我们在摄像机教程中做的类似。

		//已知上向量是表面的法线向量。右和前向量是切线(Tagent)和副切线(Bitangent/Binormal)向量。
		//下面的图片展示了一个表面的三个向量：https://learnopengl-cn.github.io/img/05/04/normal_mapping_tbn_vectors.png

		//计算出切线和副切线并不像法线向量那么容易。
		//从图中可以看到法线贴图的切线和副切线与纹理坐标的两个方向对齐。
		//我们就是用到这个特性计算每个表面的切线和副切线的。
		//需要用到一些数学才能得到它们；请看下图：https://learnopengl-cn.github.io/img/05/04/normal_mapping_surface_edges.png

		/*
		注意上图中边E2与纹理坐标的差ΔU2、ΔV2构成一个三角形。
		ΔU2与切线向量T方向相同，而ΔV2与副切线向量B方向相同。
		这也就是说，所以我们可以将三角形的边E1与E2写成切线向量\T和副切线向量B的线性组合：
		E1 = ΔU1T + ΔV1B
		E2 = ΔU2T + ΔV2B
		...
		然后我们得到两个未知数（切线T和副切线B）和两个等式。
		你可能想起你的代数课了，这是让我们去解T和B。
		
		上面的方程允许我们把它们写成另一种格式：矩阵乘法
		| E1x E1y E1z | = |ΔU1 ΔV1 |  | Tx Ty Tz |
		| E2x E2y E2z |   |ΔU2 ΔV2 |  | Bx By Bz |
		尝试会意一下矩阵乘法，它们确实是同一种等式。
		把等式写成矩阵形式的好处是，解T和B会因此变得很容易。
		两边都乘以ΔUΔV的逆矩阵等于：
		|ΔU1 ΔV1 |-1  | E1x E1y E1z | = | Tx Ty Tz |
		|ΔU2 ΔV2 |    | E2x E2y E2z |   | Bx By Bz |
		这样我们就可以解出T和B了。
		这需要我们计算出delta纹理坐标矩阵的逆矩阵。
		我不打算讲解计算逆矩阵的细节，但大致是把它变化为，1除以矩阵的行列式，再乘以它的伴随矩阵(Adjugate Matrix)。
		| Tx Ty Tz | =                          |  ΔV2  -ΔV1 | | E1x E1y E1z |
		| Bx By Bz |    1/(ΔU1ΔV2 - ΔU2ΔV1) | -ΔU2   ΔU1 | | E2x E2y E2z |
		有了最后这个等式，我们就可以用公式、三角形的两条边以及纹理坐标计算出切线向量T和副切线B。
		
		如果你对这些数学内容不理解也不用担心。
		当你知道我们可以用一个三角形的顶点和纹理坐标（因为纹理坐标和切线向量在同一空间中）计算出切线和副切线你就已经部分地达到目的了
		（译注：上面的推导已经很清楚了，如果你不明白可以参考任意线性代数教材，就像作者所说的记住求得切线空间的公式也行，不过不管怎样都得理解切线空间的含义）。
		*/
	}
	...
	void how()
	{
		//这个教程的demo场景中有一个简单的2D平面，它朝向正z方向。
		//这次我们会使用切线空间来实现法线贴图，所以我们可以使平面朝向任意方向，法线贴图仍然能够工作。
		//使用前面讨论的数学方法，我们来手动计算出表面的切线和副切线向量。
		...
		//在平面上显示出来TBN应该是这样的：https://learnopengl-cn.github.io/img/05/04/normal_mapping_tbn_shown.png
	...
	void how()
	{
		...
		*/
		...
		//从技术上讲，顶点着色器中无需副切线。
		//所有的这三个TBN向量都是相互垂直的所以我们可以在顶点着色器中用T和N向量的叉乘，自己计算出副切线：vec3 B = cross(T, N);

		//现在我们有了TBN矩阵，如果来使用它呢？
		//通常来说有两种方式使用它，我们会把这两种方式都说明一下：
		/*
		...
		*/
		...
		//我们来看看第一种情况。
		/*
		我们从法线贴图采样得来的法线向量，是在切线空间表示的，尽管其他光照向量都是在世界空间表示的。
		把TBN传给像素着色器，我们就能将采样得来的切线空间的法线乘以这个TBN矩阵，将法线向量变换到和其他光照向量一样的参考空间中。
		...
		因为光照代码就是假设法线向量在世界空间中。
		*/

		//我们同样看看第二种情况。
		/*
		我们用TBN矩阵的逆矩阵将所有相关的世界空间向量转变到采样所得法线向量的空间：切线空间。
		TBN的建构还是一样，但我们在将其发送给像素着色器之前先要求逆矩阵：
		vs_out.TBN = transpose(mat3(T, B, N));

		注意，这里我们使用transpose函数，而不是inverse函数。
		正交矩阵（每个轴既是单位向量同时相互垂直）的一大属性是一个正交矩阵的置换矩阵与它的逆矩阵相等。
		这个属性很重要因为逆矩阵的求得比求置换开销大；结果却是一样的。

		在像素着色器中我们不用对法线向量变换，但我们要把其他相关向量转换到切线空间，它们是lightDir和viewDir。
		这样每个向量还是在同一个空间（切线空间）中了。
		...
		基本上，不需要把任何向量在像素着色器中进行变换，而第一种方法中就是必须的，因为采样出来的法线向量对于每个像素着色器都不一样。

		所以现在不是把TBN矩阵的逆矩阵发送给像素着色器，而是将切线空间的光源位置，观察位置以及顶点位置发送给像素着色器。
		...
		在像素着色器中我们使用这些新的输入变量来计算切线空间的光照。
		因为法线向量已经在切线空间中了，光照就有意义了。
	...
	void how()
	{
		...
		//我们可以通过下面的代码用Assimp获取计算出来的切线空间：4
		/*
		Mesh mesh();
		...
		*/
		...
		//然后，你还必须更新模型加载器，用以从带纹理模型中加载法线贴图。
		...
		*/
		...
		//同样了解aiProcess_CalcTangentSpace并不能总是很好的工作也很重要。
		//计算切线是需要根据纹理坐标的，有些模型制作者使用一些纹理小技巧比如镜像一个模型上的纹理表面时也镜像了另一半的纹理坐标；
		//这样当不考虑这个镜像的特别操作的时候（Assimp就不考虑）结果就不对了。

		//运行程序，用新的模型加载器，加载一个有specular和法线贴图的模型，看起来会像这样：
		// https://learnopengl-cn.github.io/img/05/04/normal_mapping_complex_compare.png
		//你可以看到在没有太多点的额外开销的情况下法线贴图难以置信地提升了物体的细节。

		//使用法线贴图也是一种提升你的场景的表现的重要方式。
		//在使用法线贴图之前你不得不使用相当多的顶点才能表现出一个更精细的网格，但使用了法线贴图我们可以使用更少的顶点表现出同样丰富的细节。
		//下图来自Paolo Cignoni，图中对比了两种方式：https://learnopengl-cn.github.io/img/05/04/normal_mapping_comparison.png
	...
	看看最后的那个附加资源：https://www.youtube.com/watch?v=4FaWLgsctqY
	Normal Mapping Mathematics视频，里面有对这个过程的解释。
