...
//着色器文件路径
const char* L_vsPath = "E:/Learn/Deps/GLSL/5_AdvancedLighting/4/vLightShader.vs";
const char* L_fsPath = "E:/Learn/Deps/GLSL/5_AdvancedLighting/4/fLightShader.fs";
const char* vsPath =   "E:/Learn/Deps/GLSL/5_AdvancedLighting/4/vShader.vs";
const char* fsPath =   "E:/Learn/Deps/GLSL/5_AdvancedLighting/4/fShader.fs";
const char* M_vsPath = "E:/Learn/Deps/GLSL/5_AdvancedLighting/4/vModelShader.vs";
const char* M_fsPath = "E:/Learn/Deps/GLSL/5_AdvancedLighting/4/fModelShader.fs";
const char* bM_vsPath = "E:/Learn/Deps/GLSL/3_ModelLoading/3/vModelShader.vs";
const char* bM_fsPath = "E:/Learn/Deps/GLSL/3_ModelLoading/3/fModelShader.fs";
//摄像机
...
int main()
{
	...
	Shader bModelShader(bM_vsPath, bM_fsPath);

	//加载模型
	Model aModel("E:/Learn/Deps/Sources/objects/cyborg/cyborg.obj");
	...
	//加载纹理
	GLuint brickDiffTexture = loadTexture("E:/Learn/Deps/Sources/textures/brickwall.jpg", false);
	GLuint brickNormTexture = loadTexture("E:/Learn/Deps/Sources/textures/brickwall_normal.jpg", false);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians((GLfloat)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
		...
		//aModel（加法线贴图，布林冯模型，无光线衰弱）
		aModelShader.use();

		aModelShader.setVec3("viewPos", camera.Position);
		aModelShader.setVec3("lightPos", lightPos);
		...
		aModelShader.setMat4("projection", projection);
		aModelShader.setMat4("view", view);
		aModelShader.setMat4("model", model);
		aModel.Draw(aModelShader);

		//bModel（未加法线贴图，冯模型，光线衰弱）//对比一下模型细节就好，光照效果没统一
		bModelShader.use();

		bModelShader.setVec3("viewPos", camera.Position);
		bModelShader.setFloat("shininess", 32.0f);
		bModelShader.setVec3("light.position", lightPos);
		bModelShader.setVec3("light.ambient", 0.05f, 0.05f, 0.05f);
		bModelShader.setVec3("light.diffuse", 1.0f, 1.0f, 1.0f);
		bModelShader.setVec3("light.specular", 0.5f, 0.5f, 0.5f);
		bModelShader.setFloat("light.constant", 1.0f);
		bModelShader.setFloat("light.linear", 0.007f);
		bModelShader.setFloat("light.quadratic", 0.0002f);

		projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
		view = camera.GetViewMatrix();
		bModelShader.setMat4("projection", projection);
		bModelShader.setMat4("view", view);
		...
		bModelShader.setMat4("model", model);
		aModel.Draw(bModelShader);
