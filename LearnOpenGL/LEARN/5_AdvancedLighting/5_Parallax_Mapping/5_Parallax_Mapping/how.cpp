...
namespace how
{
	...
	void what()
	{
		...
		//和法线贴图一样视差贴图能够极大提升表面细节，使之具有深度感。
		//它也是利用了视错觉，然而对深度有着更好的表达，与法线贴图一起用能够产生难以置信的效果。
		...
		//这里粗糙的红线代表高度贴图中的数值的立体表达，向量V¯代表观察方向。
		...
		//视差贴图尝试通过对从fragment到观察者的方向向量V¯进行缩放的方式解决这个问题，
		//缩放的大小是A处fragment的高度。
		//所以我们将V¯的长度缩放为高度贴图在点A处H(A)采样得来的值。
		//下图展示了经缩放得到的向量P¯：
		// https://learnopengl-cn.github.io/img/05/05/parallax_mapping_scaled_height.png

		//我们随后选出P¯以及这个向量与平面对齐的坐标作为纹理坐标的偏移量。
		//这能工作是因为向量P¯是使用从高度贴图得到的高度值计算出来的，
		//所以一个fragment的高度越高位移的量越大。

		//这个技巧在大多数时候都没问题，但点B是粗略估算得到的(即近似结果)。
		//当表面的高度变化很快的时候，会看起来就不会真实，因为向量P¯最终不会和B接近，就像下图这样：
		// https://learnopengl-cn.github.io/img/05/05/parallax_mapping_incorrect_p.png

		//视差贴图的另一个问题是，当表面被任意旋转以后很难指出从P¯获取哪一个坐标。
		//我们在视差贴图中使用了另一个坐标空间，这个空间P¯向量的x和y元素总是与纹理表面对齐。
		//如果你看了法线贴图教程，你也许猜到了，我们实现它的方法，
		//是的，我们还是在切线空间中实现视差贴图。

		//将fragment到观察者的向量V¯转换到切线空间中，经变换的P¯向量的x和y元素将于表面的切线和副切线向量对齐。
		//由于切线和副切线向量与表面纹理坐标的方向相同，我们可以用P¯的x和y元素作为纹理坐标的偏移量，这样就不用考虑表面的方向了。
	...
	void how()
	{
		...
		//法线贴图通常根据高度贴图生成，法线贴图和高度贴图一起用能保证光照能和位移相匹配。
		...
		//我们再次获得A和B，但是这次我们用向量V¯减去点A的纹理坐标得到P¯。
		//我们通过在着色器中用1.0减去采样得到的高度贴图中的值来取得深度值，而不再是高度值，
		//或者简单地在图片编辑软件中把这个纹理进行反色操作，就像我们对连接中的那个深度贴图所做的一样。

		//位移贴图是在像素着色器中实现的，因为三角形表面的所有位移效果都不同。
		//在像素着色器中我们将需要计算fragment到观察者到方向向量V¯所以我们需要观察者位置和在切线空间中的fragment位置。
		...
		*/
		...
		//这个函数返回经位移的纹理坐标。
		//然后我们使用这些经位移的纹理坐标进行diffuse和法线贴图的采样。
		...
		*/
		...
		//所以，从本质上，相比正朝向表面，当带有角度地看向平面时，我们会更大程度地缩放P¯的大小，从而增加纹理坐标的偏移；
		//这样做在视角上会获得更大的真实度。
		
		//有些人更喜欢不在等式中使用viewDir.z，因为普通的视差贴图会在角度上产生不尽如人意的结果；这
		//个技术叫做有偏移量限制的视差贴图（Parallax Mapping with Offset Limiting）。
		...
		//这里你会看到只用法线贴图和与视差贴图相结合的法线贴图的不同之处。
	...
	void how()
	{
		...
		//从每个层中我们沿着P¯方向移动采样纹理坐标，直到我们找到一个采样低于当前层的深度值。
		//看看下面的图片：
		...
		//如果这个层的深度值小于深度贴图的值，就意味着这一层的P¯向量部分在表面之下。
		//我们继续这个处理过程直到有一层的深度高于储存在深度贴图中的值：
		//这个点就在（经过位移的）表面下方。

		//这个例子中我们可以看到第二层(D(2) = 0.73)的深度贴图的值仍低于第二层的深度值0.4，所以我们继续。
		//下一次迭代，这一层的深度值0.6大于深度贴图中采样的深度值(D(3) = 0.37)。
		//我们便可以假设第三层向量P¯是可用的位移几何位置。
		//我们可以用从向量P3¯的纹理坐标偏移T3来对fragment的纹理坐标进行位移。
	...
	void how()
	{
		...
		//你可以看到大部分和陡峭视差映射一样，不一样的地方是有个额外的步骤，
		//两个深度层的纹理坐标围绕着交叉点的线性插值。
		//这也是近似的，但是比陡峭视差映射更精确。
		...
		*/
		...
		//但这仍是一个好买卖，因为除非是放得非常大或者观察角度特别陡，否则也看不到。
	...
	//这样视差贴图的不自然也就很难能被注意到了，对于提升物体的细节可以起到难以置信的效果。
}