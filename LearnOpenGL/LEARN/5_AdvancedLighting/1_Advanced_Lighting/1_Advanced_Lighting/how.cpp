...
namespace how
{
	//在光照小节中，我们简单地介绍了冯氏光照模型，它让我们的场景有了一定的真实感。
	//虽然冯氏模型看起来已经很不错了，但是使用它的时候仍然存在一些细节问题，我们将在这一节里讨论它们。

	//布林冯模型Blinn-Phong-----------------------------------------------------
	
	//冯氏光照不仅对真实光照有很好的近似，而且性能也很高。
	//但是它的镜面反射会在一些情况下出现问题，特别是物体反光度很低时，会导致大片（粗糙的）高光区域。
	//下面这张图展示了当反光度为1.0时地板会出现的效果：https://learnopengl-cn.github.io/img/05/01/advanced_lighting_phong_limit.png
	...
	//看一下下面这两张图：https://learnopengl-cn.github.io/img/05/01/advanced_lighting_over_90.png

	//现在问题就应该很明显了。左图中是我们熟悉的冯氏光照中的反射向量，其中theta角小于90度。
	//而右图中，视线与反射方向之间的夹角明显大于90度，这种情况下镜面光分量会变为0.0。
	//这在大多数情况下都不是什么问题，因为观察方向离反射方向都非常远。
	//然而，当物体的反光度非常小时，它产生的镜面高光半径足以让这些相反方向的光线对亮度产生足够大的影响。
	//在这种情况下就不能忽略它们对镜面光分量的贡献了。
	...
	*/
	...
	//除此之外Blinn-Phong模型就没什么好说的了，Blinn-Phong与冯氏模型唯一的区别就是，
	//Blinn-Phong测量的是法线与半程向量之间的夹角，而冯氏模型测量的是观察方向与反射向量间的夹角。

	//在引入半程向量之后，我们现在应该就不会再看到冯氏光照中高光断层的情况了。
	//下面两个图片展示的是两种方法在镜面光分量为0.5时的对比：https://learnopengl-cn.github.io/img/05/01/advanced_lighting_comparrison.png

	//除此之外，冯氏模型与Blinn-Phong模型也有一些细微的差别：
	//半程向量与表面法线的夹角通常会小于观察与反射向量的夹角。
	//所以，如果你想获得和冯氏着色类似的效果，就必须在使用Blinn-Phong模型时将镜面反光度设置更高一点。
	...
	//你可以看到，Blinn-Phong的镜面光分量会比冯氏模型更锐利一些。
