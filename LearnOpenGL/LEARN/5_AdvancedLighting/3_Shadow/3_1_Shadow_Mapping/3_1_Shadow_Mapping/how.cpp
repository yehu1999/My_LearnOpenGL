...
unsigned int lightSpaceMatrixLocation, depthMapFBO;
...
namespace how
{
	...
	void how()
	{
		...
		//这里的所有蓝线代表光源可以看到的fragment。
		//黑线代表被遮挡的fragment：它们应该渲染为带阴影的。
		//如果我们绘制一条从光源出发，到达最右边盒子上的一个片段上的线段或射线，那么射线将先击中悬浮的盒子，随后才会到达最右侧的盒子。
		//结果就是悬浮的盒子被照亮，而最右侧的盒子将处于阴影之中。
		...
		//我们可以采取相似举措，不用投射出光的射线。
		//我们所使用的是非常熟悉的东西：深度缓冲。

		//你可能记得在深度测试教程中，在深度缓冲里的一个值是摄像机视角下，对应于一个片段的一个0到1之间的深度值。
		//如果我们从光源的透视图来渲染场景，并把深度值的结果储存到纹理中会怎样？
		//通过这种方式，我们就能对光源的透视图所见的最近的深度值进行采样。
		//最终，深度值就会显示从光源的透视图下见到的第一个片段了。
		...
		//我们使用一个来自光源的视图和投影矩阵来渲染场景就能创建一个深度贴图。
		//这个投影和视图矩阵结合在一起成为一个T变换，它可以将任何三维位置转变到光源的可见坐标空间。

		//定向光并没有位置，因为它被规定为无穷远。
		//然而，为了实现阴影贴图，我们得从一个光的透视图渲染场景，这样就得在光的方向的某一点上渲染场景。

		//在右边的图中我们显示出同样的平行光和观察者。
		//我们渲染一个点P¯处的片段，需要决定它是否在阴影中。
		//我们先得使用T把P¯变换到光源的坐标空间里。
		//既然点P¯是从光的透视图中看到的，它的z坐标就对应于它的深度，例子中这个值是0.9。
		//使用点P¯在光源的坐标空间的坐标，我们可以索引深度贴图，来获得从光的视角中最近的可见深度，结果是点C¯，最近的深度是0.4。
	...
	void how()
	{
		...
		//这里一定要记得调用glViewport。因为阴影贴图经常和我们原来渲染的场景（通常是窗口分辨率）有着不同的分辨率，我们需要改变视口（viewport）的参数以适应阴影贴图的尺寸。
	...
	void how()
	{
		...
		//这里有个本节教程的demo场景中使用的正交投影矩阵的例子。
		//因为投影矩阵间接决定可视区域的范围，以及哪些东西不会被裁切，
		//你需要保证投影视锥（frustum）的大小，以包含打算在深度贴图中包含的物体。
		//当物体和片段不在深度贴图中时，它们就不会产生阴影。

		//为了创建一个视图矩阵来变换每个物体，把它们变换到从光源视角可见的空间中，
		//我们将使用glm::lookAt函数；这次从光源的位置看向场景中央。
		...
		//然而，我们只关心深度值，并非所有片段计算都在我们的着色器中进行。
		//为了提升性能，我们将使用一个与之不同但更为简单的着色器来渲染出深度贴图。
	}
	...
	void how()
	{
		...
		//这个简单的着色器叫做simpleDepthShader，就是使用下面的这个着色器：
		/*
		#version 330 core
		...
		*/
		...
		//我们可以取消那行的注释，来显式设置深度，但是这个（指注释掉那行之后）是更有效率的，
		//因为底层无论如何都会默认去设置深度缓冲。
		...
		//这里的RenderScene函数的参数是一个着色器程序（shader program），
		//它调用所有相关的绘制函数，并在需要的地方设置相应的模型矩阵。

		//最后，在光的透视图视角下，很完美地用每个可见片段的最近深度填充了深度缓冲。
		//通过将这个纹理投射到一个2D四边形上（和我们在帧缓冲一节做的后处理过程类似），
	...
	void how()
	{
		...
		*/
		...
		//片段着色器大部分是从高级光照教程中复制过来，只不过加上了个阴影计算。
		//我们声明一个shadowCalculation函数，用它计算阴影。
		//片段着色器的最后，我们我们把diffuse和specular乘以(1-阴影元素)，这表示这个片段有多大成分不在阴影中。(不在阴影才绘制)
		//这个片段着色器还需要两个额外输入，一个是光空间的片段位置和第一个渲染阶段得到的深度贴图。

		//要检查一个片段是否在阴影中，首先要把光空间片段位置转换为裁切空间的标准化设备坐标。
		...
		*/
		...
		//可是，当使用透视投影的时候就是必须的了，所以为了保证在两种投影矩阵下都有效就需要这行代码。

		//因为来自深度贴图的深度在0到1的范围，我们也打算使用projCoords从深度贴图中去采样，
		//所以我们将NDC坐标变换为0到1的范围。 
		//（译者注：这里的意思是，上面的projCoords的xyz分量都是[-1,1]（下面会指出这对于远平面之类的点才成立），
	...
	void how()
	{
		//我们试图让阴影映射工作，但是你也看到了，阴影映射还是有点不真实，
		//我们修复它才能获得更好的效果，这是下面的部分所关注的焦点。
		...
		//图片每个斜坡代表深度贴图一个单独的纹理像素。
		//你可以看到，多个片段从同一个深度值进行采样。

		//虽然很多时候没问题，但是当光源以一个角度朝向表面的时候就会出问题，
		//这种情况下深度贴图也是从一个角度下进行渲染的。
		//多个片段就会从同一个斜坡的深度纹理像素中采样，有些在地板上面，有些在地板下面；
		//这样我们所得到的阴影就有了差异。因为这个，有些片段被认为是在阴影之中，有些不在，由此产生了图片中的条纹样式。
		// （个人觉得更好的解释） https://www.zhihu.com/question/49090321/answer/114217482

		//我们可以用一个叫做阴影偏移（shadow bias）的技巧来解决这个问题，
		//我们简单的对表面的深度（或深度贴图）应用一个偏移量，这样片段就不会被错误地认为在表面之下了。
		//（但是这也会导致阴影悬浮）
		//使用了偏移量后，所有采样点都获得了比表面深度更小的深度值，
		//这样整个表面就正确地被照亮，没有任何阴影。
		//我们可以这样实现这个偏移：
		/*
		float bias = 0.005;
		...
		*/
		...
		//选用正确的偏移数值，在不同的场景中需要一些像这样的轻微调校，
		//但大多情况下，实际上就是增加偏移量直到所有失真都被移除的问题。

		//阴影悬浮-------
		//使用阴影偏移的一个缺点是你对物体的实际深度应用了平移。
		//偏移有可能足够大，以至于可以看出阴影相对实际物体位置的偏移。

		//这个阴影失真叫做悬浮(Peter Panning)，因为物体看起来轻轻悬浮在表面之上
		//（译注Peter Pan就是童话彼得潘，而panning有平移、悬浮之意，而且彼得潘是个会飞的男孩…）。
		//我们可以使用一个叫技巧解决大部分的Peter panning问题：
		//当渲染深度贴图时候使用正面剔除（front face culling）你也许记得在面剔除教程中OpenGL默认是背面剔除。
		//我们要告诉OpenGL我们要剔除正面。
		...
		//。地面是一个单独的平面，不会被完全剔除。
		//如果有人打算使用这个技巧解决peter panning必须考虑到只有剔除物体的正面才有意义。
		...
		//采样过多-------
		//无论你喜不喜欢还有一个视觉差异，就是光的视锥不可见的区域一律被认为是处于阴影中，不管它真的处于阴影之中。
		//出现这个状况是因为超出光的视锥的投影坐标比1.0大，这样采样的深度纹理就会超出他默认的0到1的范围。
		//根据纹理环绕方式，我们将会得到不正确的深度结果，它不是基于真实的来自光源的深度值。

		//你可以在图中看到，光照有一个区域，超出该区域就成为了阴影；
		//这个区域实际上代表着深度贴图的大小，这个贴图投影到了地板上。
		...
		*/
		...
		//另一个（并不完整的）解决方案叫做PCF（percentage-closer filtering），
		//这是一种多个不同过滤方式的组合，它产生柔和阴影，使它们出现更少的锯齿块和硬边。
		...
		*/
		...
		//实际上PCF还有更多的内容，以及很多技术要点需要考虑以提升柔和阴影的效果，
		//但出于对本章内容长度的考虑，我们将留在以后讨论。
		...
		//下图展示了两种投影方式所产生的不同阴影区域：https://learnopengl-cn.github.io/img/05/03/01/shadow_mapping_projection.png
