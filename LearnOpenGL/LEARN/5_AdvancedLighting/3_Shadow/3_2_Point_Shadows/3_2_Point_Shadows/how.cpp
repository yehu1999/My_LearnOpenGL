...
namespace how
{
	...
	//算法和定向阴影映射差不多：我们从光的透视图生成一个深度贴图，基于当前fragment位置来对深度贴图采样，
	//然后用储存的深度值和每个fragment进行对比，看看它是否在阴影中。
	//定向阴影映射和万向阴影映射的主要不同在于深度贴图的使用上。

	//对于深度贴图，我们需要从一个点光源的所有渲染场景，普通2D深度贴图不能工作；
	//如果我们使用立方体贴图会怎样？
	//因为立方体贴图可以储存6个面的环境数据，它可以将整个场景渲染到立方体贴图的每个面上，
	//把它们当作点光源四周的深度值来采样。
	...
	//大部分复杂的事情已经在阴影映射教程中讨论过了。
	//算法只是在深度立方体贴图生成上稍微复杂一点。
	...
	void how()
	{
		...
		//这个过程和默认的阴影映射一样，尽管这次我们渲染和使用的是一个立方体贴图深度纹理，而不是2D深度纹理。
		//在我们实际开始从光的视角的所有方向渲染场景之前，我们先得计算出合适的变换矩阵。
	}
	...
	void how()
	{
		...
		//glm::lookAt的target参数是它注视的立方体贴图的面的一个方向。
	...
	void how()
	{
		//为了把值渲染到深度立方体贴图，我们将需要3个着色器：
		//顶点和片段着色器，以及一个它们之间的几何着色器。
		...
		*/
		...
		//几何着色器接下来会负责将顶点变换到光空间；这里它开始变得有趣了。
		...
		*/
		...
		//上个教程，我们使用的是一个空的像素着色器，让OpenGL配置深度贴图的深度值。
	...
	void how()
	{
		...
		//这个过程和定向阴影映射教程相似，
		//尽管这次我们绑定的深度贴图是一个立方体贴图，而不是2D纹理，并且将光的投影的远平面发送给了着色器。
		glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT);
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		shader.use();
		...
		//这里的renderScene函数将渲染一个大立方体房间与一些立方体，它们散落在大立方体各处，光源在场景中央。

		//顶点着色器和像素着色器和原来的阴影映射着色器大部分都一样：
		//不同之处是在光空间中像素着色器不再需要一个fragment位置，现在我们可以使用一个方向向量采样深度值。
		...
		*/
		...
		//在ShadowCalculation函数中有很多不同之处，现在是从立方体贴图中进行采样，不再使用2D纹理了。
		//我们来一步一步的讨论一下的它的内容。

		//我们需要做的第一件事是获取立方体贴图的深度。
		//你可能已经从教程的立方体贴图部分想到，我们已经将深度储存为fragment和光位置之间的距离了；
		//我们这里采用相似的处理方式：
		/*
		float ShadowCalculation(vec3 fragPos)
		{
		...
		*/
		//在这里，我们得到了fragment的位置与光的位置之间的差向量，
		...
		//下一步我们获取当前fragment和光源之间的深度值，我们可以简单的使用fragToLight的长度来获取它，
		//这取决于我们如何计算立方体贴图中的深度值：
	...
	void how()
	{
		//如果你想我一样第一次并没有做对，那么就要进行调试排错，将深度贴图显示出来以检查其是否正确。
		//因为我们不再用2D深度贴图纹理，深度贴图的显示不会那么显而易见。
		...
		//你可能也注意到了带阴影部分在墙外。如果看起来和这个差不多，你就知道深度立方体贴图生成的没错。
		//否则你可能做错了什么，也许是closestDepth仍然还在0到far_plane的范围。
	}
	...
	//这取决于环境的类型，以及特定的显卡驱动等等，所以如果你很关心性能，就要确保对两种方法有大致了解，然后选择对你场景来说更高效的那个。
